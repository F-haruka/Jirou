...
{
    [Header("楽曲情報")]
    public AudioClip songClip;          // 楽曲ファイル
    public float bpm = 120f;            // BPM
    public string songName;             // 曲名
    public string artist;               // アーティスト名
    public float previewTime = 0f;      // プレビュー開始時間
    public float firstBeatOffset = 0f;  // 最初のビートまでのオフセット
    
    [Header("難易度情報")]
    public int difficulty = 1;          // 難易度（1-10）
    public string difficultyName = "Normal";  // 難易度名
    
    [Header("譜面データ")]
    public List<NoteData> notes = new List<NoteData>();
    
    [Header("メタデータ")]
    public string chartAuthor;          // 譜面作成者
    public string chartVersion = "1.0"; // 譜面バージョン
    public DateTime createdDate;        // 作成日時
...
public class ChartData : ScriptableObject
{
    ...
    public float GetChartLengthInBeats()
    {
        ...
        float lastBeat = notes.Max(n => 
            n.noteType == NoteType.Hold ? 
            n.timeToHit + n.holdDuration : 
            n.timeToHit);
...
public class ChartData : ScriptableObject
{
    ...
    public bool ValidateChart(out List<string> errors)
    {
        ...
        for (int i = 0; i < notes.Count; i++)
        {
            ...
            // レーンインデックスチェック
            if (note.laneIndex < 0 || note.laneIndex > 3)
            {
                errors.Add($"ノーツ{i}: 不正なレーンインデックス {note.laneIndex}");
                isValid = false;
            }
            ...
            if (note.timeToHit < 0)
            {
                errors.Add($"ノーツ{i}: 負のタイミング値 {note.timeToHit}");
                isValid = false;
            }
            ...
            {
                errors.Add($"ノーツ{i}: Holdノーツの長さが不正 {note.holdDuration}");
                isValid = false;
            }
            
            // スケールチェック
            if (note.visualScale <= 0)
            {
                errors.Add($"ノーツ{i}: 不正なスケール値 {note.visualScale}");
                isValid = false;
            }
        }
